export async function onRequestPost(context) {
  try {
    ...
    // Human-readable labels for emails
    ...
    // ── Internal notification to Joseph ──────────────────────────────
    const internalHtml = `
    ...
    const clientHtml = `
      <div style="font-family:sans-serif;background:#0b0f14;color:#e2e8f0;padding:40px;max-width:560px;margin:0 auto;border-radius:12px">
        ...
        <div style="border-top:1px solid #1e293b;padding-top:24px">
          ...
          <p style="margin:16px 0 0;font-size:14px;color:#94a3b8">— Joseph, Eldo Network<br><a href="mailto:hello@eldo.network" style="color:#f1c453">hello@eldo.network</a></p>
        </div>
      </div>
    `;
    ...
    if (resendKey && email) {
      await Promise.allSettled([
        fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: "Eldo Network <hello@eldo.network>",
            to: ["hello@eldo.network"],
            reply_to: email,
